...
12. **Scalar component vs. projection vs. dot product?**  
    Scalar component = signed **HOW MUCH** of $v$ lies along $u$. Projection = **HOW MUCH × WHICH DIRECTION**, so it is a vector. Dot product = $‖u‖ ×$ that scalar component.

13. **Why can equal dot products still mean different alignment?**  
    Magnitude and angle can compensate: $u·v=‖u‖‖v‖cosθ$. Equal raw dots do not imply equal cosine similarity.

14. **What does orthogonal mean?**  
    Perpendicular: $u·v = 0$ (for nonzero vectors). Their magnitudes do not change the zero result.

15. **What is magnitude?**  
    $‖v‖ = √(v·v) = √(Σvᵢ²)$. It is the arrow’s length—not a “distance between vectors.”

16. **Why is $v·v = ‖v‖²$?**  
    A vector has angle $0°$ with itself, so $v·v = ‖v‖‖v‖cos0 = ‖v‖²$.

17. **What is the span of vectors?**  
...
19. **Dimension rule for multiplication?**  
...
- **Trigger:** “Projection/component/dot product are the same.”  
  **Redirect:** Component = scalar HOW MUCH; projection = vector HOW MUCH × WHICH DIRECTION; dot = reference magnitude × component.
